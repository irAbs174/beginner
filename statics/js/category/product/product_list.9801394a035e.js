$(document).ready(function(){
    function load_products(){
        ...
        $.getJSON(`/UNIQUEAPI174/pages/?type=category.CategoryProduct&id=${slug}`,function(tstData){
          console.log(tstData);
          if(tstData.items.length > 0){
            ...
            Swal.fire({
              icon: "error",
              title: 'محصولی با این دسته بندی یافت نشد',
        ...
        $.getJSON(`/UNIQUEAPI174/pages/?type=product.InventoryItem`, function(data){
            ...
            for (var i = startIndex; i < endIndex; i++) {
              ...
              $.getJSON(brands_API, function(brandData){
                product = brandData;
                for (let p = 0; p < product.collection.length; p++){
                  if (product.collection[p].id == slug){
                    ...
                    `;
                      $('#PRODUCT_CATEGORY_LIST').append(postHTML);
                  }else{
                      var postHTML = `
