...
def fadax_return_shop(request):
    if request.method == 'POST':
        ...
        success = received_data['success']
        if success:
            transactionId = received_data['response']['transactionId']
            customer = Fadax_payment.objects.filter(transactionId=transactionId)[0].customer
            paymentToken = Fadax_payment.objects.filter(transactionId=transactionId)[0].paymentToken
            url = f"https://{fadax_API_PATH}/supplier/v1/verify’"
            headers = {
                "accept": "application/json",
                "Authorization": f"Bearer {fadax_API}",
